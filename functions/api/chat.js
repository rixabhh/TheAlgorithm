...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const toneGuidance =
      {
        playful:
          "Playful: witty, relatable, and light, but still anchored to the user's actual data.",
        balanced:
          "Balanced: warm, specific, emotionally intelligent, and easy to act on.",
...
-------------------------`;
    ...
    try {
      responseText = await makeChatLLMCall(
        provider,
        api_key,
        systemPrompt,
        chat_history,
    ...
    if (!responseText) {
      responseText =
        "I'm having trouble connecting to my models right now. Please try again later.";
    }
    ...
    let errorMsg =
      e.message || "Chat failed. Check your API key and try again.";
