...
export async function makeChatLLMCall(
  provider,
  api_key,
  systemPrompt,
  chat_history,
  ...
  return callOpenAICompatible(provider, api_key, messages, env, signal, {
    max_tokens: 500,
  });
}
...
async function makeFreeChatCall(
  apiKey,
  systemPrompt,
  chatHistory,
  message,
  ...
  if (openRouterKey) {
    ...
    try {
      ...
      console.warn(
        "OpenRouter free chat failed, falling back to Cloudflare AI:",
        err,
      );
    }
  }
  ...
  if (env.AI)
    return makeCloudflareChatCall(systemPrompt, chatHistory, message, env);
  return null;
}

async function makeFreeAnalysisCall(
  apiKey,
  systemPrompt,
  userPrompt,
  env,
  ...
  if (openRouterKey) {
    try {
      ...
      console.warn(
        "OpenRouter free analysis failed, falling back to Cloudflare AI:",
        err,
      );
    }
  }
