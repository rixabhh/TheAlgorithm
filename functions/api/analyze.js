...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (raw_excerpt_pack && privacy_mode !== "opt_in_raw") {
      return new Response(
        JSON.stringify({
          error: "Raw excerpts require opt-in raw evidence mode.",
        }),
        { status: 400 },
      );
    }
    ...
    const sourceScore = clampHeuristic(
      source_quality?.score || evidence_pack?.predictive_outlook?.confidence,
      60,
    );
    ...
    if (freeTierProviders.has(provider) && env.KV_RATELIMIT) {
      ...
      if (count >= 2)
        return new Response(
          JSON.stringify({
            error: "Free tier limit reached (2/hr). Wait or use BYOK.",
          }),
          { status: 429 },
        );
      await env.KV_RATELIMIT.put(limitKey, (count + 1).toString(), {
        expirationTtl: 3600,
      });
      ...
      await env.KV_RATELIMIT.put(
        globalCountKey,
        (parseInt(globalCount) + 1).toString(),
      );
    }
    ...
    const makeFallbackReport = () => {
      const totalMessages =
        ...
        (stats?.messages?.ME || 0) + (stats?.messages?.PARTNER || 0);
      const messageSplit = `${stats?.messages?.ME || 0} vs ${stats?.messages?.PARTNER || 0}`;
      const dropOffRisk = clampHeuristic(
        evidence_pack?.predictive_outlook?.drop_off_risk ?? 100 - symmetryScore,
        30,
      );
      const stability = clampHeuristic(
        evidence_pack?.predictive_outlook?.stability ?? 100 - dropOffRisk,
        65,
      );
      const healthScore = clampHeuristic(
        symmetryScore * 0.75 + sourceScore * 0.25,
        75,
      );
      const providedReceipts = Array.isArray(evidence_pack?.receipts)
        ? evidence_pack.receipts.filter(Boolean)
        : [];
      const localRiskClaim =
        dropOffRisk >= 65
      ...
      const fallbackReceipt = {
        ...
        action:
          "Use the strongest repeated signal to ask for one clear change, then watch whether the pattern improves.",
      };
      const hasProvidedReceipts = providedReceipts.length > 0;
      const receipts = hasProvidedReceipts
        ? providedReceipts
        : [fallbackReceipt];
      const topReceipt = receipts[0] || fallbackReceipt;
      const nextAction =
        evidence_pack?.predictive_outlook?.next_action ||
        topReceipt?.action ||
        fallbackReceipt.action;
      const patternName = String(topReceipt?.pattern || "local_stats").replace(
        /_/g,
        " ",
      );
      const attachmentNote =
        stats?.attachment_style ||
        "Attachment read is inferred from aggregate timing, balance, and repeated-response patterns only.";
      return {
        relationship_persona: topReceipt?.pattern
          ? `${topReceipt.pattern.replace(/_/g, " ")} watcher`
          : "Pattern Watch",
        compatibility_score: clampHeuristic(
          symmetryScore * 0.7 + sourceScore * 0.3,
          75,
        ),
        ...
        humor_dynamics: {
          fun_person:
            (stats?.laughter?.ME || 0) >= (stats?.laughter?.PARTNER || 0)
              ? my_name
              : partner_name,
        ...
        silence_breaking: {
          ice_breaker:
            (stats?.silence_breakers?.ME || 0) >=
            (stats?.silence_breakers?.PARTNER || 0)
              ? my_name
              : partner_name,
        ...
        key_insights: [
          ...
          source_quality?.warnings?.[0] ||
            `Strongest local pattern: ${patternName}.`,
        ],
        strengths: [
          `Local stats can compare message share, timing, and repeated patterns across ${totalMessages} messages.`,
        ],
        growth_areas: [
          topReceipt?.claim || `Watch the ${patternName} pattern.`,
        ],
        ...
        ai_insight: {
          ...
          dynamic_title:
            hasProvidedReceipts && topReceipt?.claim
              ? topReceipt.claim.split(" ").slice(0, 4).join(" ")
              : "Local Pattern Read",
          reality_check:
            topReceipt?.evidence ||
            `The strongest local signal is ${stats?.symmetry?.label || "balanced"} participation.`,
          recent_shift: evidence_pack?.predictive_outlook?.trend
            ? `Recent trend appears ${evidence_pack.predictive_outlook.trend}.`
            : "Recent shift is based on volume, sentiment, and response patterns.",
          red_flags: [topReceipt?.claim || `Watch the ${patternName} pattern.`],
          green_flags: topReceipt?.confidence
            ? [`Top receipt confidence is ${topReceipt.confidence}.`]
            : ["Active conversation patterns were detected."],
          brutal_verdict:
            topReceipt?.action ||
            "The data gives you a useful mirror, not a courtroom verdict.",
        },
      };
    };
    ...
    const normalizeReport = (candidate) => {
      ...
      report.communication_style = {
        ...fallback.communication_style,
        ...(candidate?.communication_style || {}),
      };
      report.attachment_style = {
        ...fallback.attachment_style,
        ...(candidate?.attachment_style || {}),
      };
      report.humor_dynamics = {
        ...fallback.humor_dynamics,
        ...(candidate?.humor_dynamics || {}),
      };
      report.silence_breaking = {
        ...fallback.silence_breaking,
        ...(candidate?.silence_breaking || {}),
      };
      report.ai_insight = {
        ...fallback.ai_insight,
      ...
      report.verdict_summary = {
        ...
        ...(candidate?.verdict_summary || {}),
      };
      report.predictive_outlook = {
        ...fallback.predictive_outlook,
        ...(candidate?.predictive_outlook || {}),
      };
      report.key_insights = Array.isArray(candidate?.key_insights)
        ? candidate.key_insights
        : fallback.key_insights;
      report.strengths = Array.isArray(candidate?.strengths)
        ? candidate.strengths
        : fallback.strengths;
      report.growth_areas = Array.isArray(candidate?.growth_areas)
        ? candidate.growth_areas
        : fallback.growth_areas;
      report.receipts =
        Array.isArray(candidate?.receipts) && candidate.receipts.length
          ? candidate.receipts
          : fallback.receipts;
      report.compatibility_score = clampHeuristic(
        report.compatibility_score,
        fallback.compatibility_score,
      );
      report.overall_health_score = clampHeuristic(
        report.overall_health_score || report.ai_insight.health_score,
        fallback.overall_health_score,
      );
      report.ai_insight.health_score = clampHeuristic(
        report.ai_insight.health_score || report.overall_health_score,
        report.overall_health_score,
      );
      report.communication_style.balance_score = clampHeuristic(
        report.communication_style.balance_score,
        fallback.communication_style.balance_score,
      );
      report.predictive_outlook.stability = clampHeuristic(
        report.predictive_outlook.stability,
        fallback.predictive_outlook.stability,
      );
      report.predictive_outlook.drop_off_risk = clampHeuristic(
        report.predictive_outlook.drop_off_risk,
        fallback.predictive_outlook.drop_off_risk,
      );
      report.predictive_outlook.confidence = clampHeuristic(
        report.predictive_outlook.confidence,
        fallback.predictive_outlook.confidence,
      );
      if (typeof report.verdict_summary.confidence === "string") {
        report.verdict_summary.confidence =
          report.verdict_summary.confidence.replace(/\b100%/g, "95+%");
      }
      return report;
    };
    ...
    const toneGuidance =
      {
        playful:
          "Playful: witty, meme-aware, light on its feet, but still useful. Use short punchy lines and one tasteful social-native phrase when it fits the data.",
        balanced:
          "Balanced: warm, clear, emotionally intelligent, and specific. Sound like a friend who can read patterns without being dramatic.",
        direct:
          "Direct: concise, sharp, no fluff. Say the pattern, why it matters, and the next move.",
      }[tone] ||
      "Balanced: warm, clear, emotionally intelligent, and specific.";
    const languageGuidance =
      {
        english: "Use natural English that feels personal, not corporate.",
        hinglish:
          "Use conversational Hinglish in English letters only. Keep it neutral and avoid gendered placeholders unless the user/source uses them.",
        hindi:
          "Use Hindi wording if requested, but keep labels and JSON keys unchanged.",
      }[String(language || "english").toLowerCase()] ||
      "Use the requested language naturally and keep JSON keys unchanged.";
...
20. Make the dashboard copy readable on mobile: short headlines, one idea per sentence, no giant paragraph blocks.`;
    ...
    const systemPrompt =
      PROVIDER_SYSTEM_PROMPTS[provider] || PROVIDER_SYSTEM_PROMPTS["default"];
...
`;
    ...
    try {
      let rawResponseText = await makeLLMCall(
        provider,
        api_key,
        systemPrompt,
        userPrompt,
      ...
      if (rawResponseText) {
        ...
        const canRetry =
          provider !== "cloudflare" &&
          (api_key || provider === "free" || provider === "openrouter_free");
        if (!report && canRetry) {
          // Try one more time with a stricter prompt if parsing failed
          const stricterUserPrompt =
            userPrompt +
          ...
          try {
            const retryResponseText = await makeLLMCall(
              provider,
              api_key,
              systemPrompt,
              stricterUserPrompt,
      ...
      if (freeTierProviders.has(provider)) {
        console.error(
          "Free analysis failed; using local evidence fallback:",
          callError,
        );
    ...
    let errorMsg =
      e.message || "Analysis failed. Check your API key and try again.";
