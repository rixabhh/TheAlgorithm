...
export async function onRequestPost(context) {
    ...
    try {
        ...
        // 1. RATE LIMITING & GLOBAL STATS (KV-based, applied globally to all requests)
        if (env.KV_RATELIMIT) {
            ...
            if (freeTierProviders.has(provider)) {
                ...
                if (count >= 50) return new Response(JSON.stringify({ error: "Rate limit reached. Please try again later." }), { status: 429 });
            }

            await env.KV_RATELIMIT.put(limitKey, (count + 1).toString(), { expirationTtl: 3600 });
            
            if (freeTierProviders.has(provider)) {
                // Increment global stats counter for free tier
        ...
        const baseSystemPrompt = `You are 'The Algorithm', a brilliant friend who happens to be a therapist - an expert relationship analyst and communication coach for new-age, social-native users. You act like a perceptive friend with data: warm, insightful, empathetic, but brutally honest when needed, emotionally sharp, funny when appropriate, and honest without being cruel.
CRITICAL RULES:
