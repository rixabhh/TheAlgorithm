...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const providerHints = {
    ...
    openrouter:
      "Use an OpenRouter key. Recommended model is openai/gpt-4o-mini.",
  ...
  const totalMessages = (stats) =>
    number(stats.messages?.ME) + number(stats.messages?.PARTNER);
  const scoreText = (value, options = {}) =>
    formatHeuristicScore(value, options);
  const scoreValue = (value, min = 5, max = 95) =>
    clampHeuristicScore(value, min, max);
  ...
  if (isCompareMode) {
    ...
    document
      .getElementById("comparison-sticky-nav")
      ?.classList.remove("hidden");
    document.getElementById("comp-name-a").textContent =
      `${dataA.my_name} & ${dataA.partner_name}`;
    document.getElementById("comp-name-b").textContent =
      `${dataB.my_name} & ${dataB.partner_name}`;
    document.getElementById("insights-heading").textContent =
      "Comparative Analysis";
    document.getElementById("insights-sub").textContent =
      "Side-by-side comparison of your chat dynamics.";
  } else {
  ...
  const setupNavigation = () => {
    ...
    const updateActiveLink = () => {
      let current = "";
      sections.forEach((s) => {
        if (window.scrollY >= s.offsetTop - 170) current = s.getAttribute("id");
      });
      navLinks.forEach((l) => {
        l.classList.remove("active");
        if (l.getAttribute("href").substring(1) === current)
    ...
    document
      .querySelectorAll(".card, .stat-card, .insight-card, .flag-box")
      .forEach((el) => {
        el.style.opacity = "0";
  ...
  const refreshAll = () => {
    ...
    setText(
      "report-headline",
      isCompareMode ? "Comparative Analysis" : `${meName} & ${partnerName}`,
    );
    const pulse = document.getElementById("report-pulse");
    if (pulse)
      pulse.textContent = `${totalMessages(stats).toLocaleString()} messages - ${stats.duration || "--"} - ${activeData.platform || "Unknown"}`;
    const historyCount = (() => {
      try {
        return JSON.parse(localStorage.getItem("algo_history") || "[]").length;
      } catch (_) {
        return 0;
      }
    })();
  ...
  const renderOverviewStats = (stats) => {
    ...
    const replyValues = [
      number(init.me_latency_avg, -1),
      number(init.partner_latency_avg, -1),
    ].filter((v) => v >= 0);
    const avgReply = replyValues.length
      ? replyValues.reduce((a, b) => a + b, 0) / replyValues.length
      : 0;
    ...
    setText(
      "stat-symmetry",
      stats.symmetry?.score !== undefined
        ? scoreText(stats.symmetry.score)
        : "--",
    );
  };
  ...
  const renderSourceQuality = (quality = {}, inputMode = "export") => {
    ...
    setText(
      ...
      `${label} - ${quality.parsed_count || totalMessages(activeData.stats || {})} parsed messages`,
    );
    setText(
      "source-quality-score",
      quality.score !== undefined ? scoreText(quality.score) : "--",
    );
    const warnings = quality.warnings || [];
    const privacy =
      activeData.privacy_mode === "opt_in_raw"
        ? "Deep AI raw evidence was enabled for this report."
        : "Stats-only AI mode: raw content stays local.";
    setText(
      "source-quality-warnings",
      [privacy, ...warnings].filter(Boolean).join(" "),
    );
  };
  ...
  const renderPatternSignals = (evidencePack = {}) => {
    ...
    const items = [
      [
        ...
        counts.unanswered_questions || 0,
        "Questions that got short answers, deflections, or topic switches.",
      ],
      [
        "Conflict Delays",
        counts.delayed_after_conflict || 0,
        "Tense messages followed by long silence or delayed replies.",
      ],
      [
        "Pressure Language",
        counts.boundary_pressure || 0,
        "Blame, pressure, or always/never framing.",
      ],
      [
        "Repair Attempts",
        counts.repair_attempts || 0,
        "Apology, repair, or reconnection language.",
      ],
    ];
    container.innerHTML = items
      .map(
        ([label, value, desc], index) => `
  ...
  const renderSocialDynamics = (stats) => {
    ...
    const update = (suffix, data) => {
      ...
      if (pills)
        pills.innerHTML = (data.highlights || [])
          .map(
            (h) =>
              `<span class="pill-label pill-label--pink" style="font-size:0.6rem">${escapeHTML(h.label)}: ${escapeHTML(String(h.count))}</span>`,
          )
          .join("");
  ...
  const renderEngagement = (stats) => {
    ...
    const starter =
      init.me_initiations === init.partner_initiations
        ? "Balanced"
        : init.me_initiations > init.partner_initiations
          ? activeData.my_name
  ...
  const renderEmoji = (stats) => {
    ...
    if (!container) return;
    const emojis = [
      ...(stats.emoji_frequency?.ME || []),
      ...(stats.emoji_frequency?.PARTNER || []),
    ]
    ...
    container.innerHTML = emojis
      .map(
        (e) =>
          `<div class="flex align-center gap-3"><span>${escapeHTML(e.emoji)}</span><div class="flex-1 h-2 bg-cream rounded-full overflow-hidden"><div class="h-full bg-pink" style="width:${barWidth((e.count / max) * 100)}"></div></div></div>`,
      )
      .join("");
  };
  ...
  const renderWordCloud = (stats) => {
    ...
    if (!words.length) {
      container.innerHTML = '<p class="op-30">No data</p>';
      return;
    }
    const max = words[0].count;
    container.innerHTML = `<div class="flex flex-wrap justify-center p-4 gap-4">${words
      .slice(0, 20)
      .map(
        (w) =>
          `<span style="font-size:${0.8 + (w.count / max) * 1.5}rem; font-weight:900; opacity:${0.4 + (w.count / max) * 0.6}">${escapeHTML(w.word)}</span>`,
      )
      .join("")}</div>`;
  };
  ...
  const initRatioChart = (stats) => {
    ...
    if (!values[0] && !values[1]) {
      document.getElementById("ratio-chart-container").innerHTML =
        '<div class="chart-empty">No ratio data available.</div>';
      return;
    }
  ...
  const initActivityChart = (weeks) => {
    ...
    if (!ctx) return;
    if (!weeks.length) {
      document.getElementById("activity-chart-container").innerHTML =
        '<div class="chart-empty">No weekly activity data available.</div>';
      return;
    }
    ...
    activityChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            label: activeData.my_name,
            data: weeks.map((w) => w.me_count),
            backgroundColor: "#000",
          },
          {
            label: activeData.partner_name,
            data: weeks.map((w) => w.partner_count),
  ...
  const initMoodChart = (weeks) => {
    ...
    if (!ctx) return;
    if (!weeks.length) {
      document.getElementById("mood-chart-container").innerHTML =
        '<div class="chart-empty">No mood timeline data available.</div>';
      return;
    }
  ...
  const setupProviderSettings = () => {
    ...
    const showModal = () => {
      ...
      apiKeyEl.value =
        storedToken && storedToken !== btoa("")
          ...
          : "";
      if (rawConsentEl)
        rawConsentEl.checked = activeData.privacy_mode === "opt_in_raw";
      refreshHint();
    ...
    saveBtn?.addEventListener("click", () => {
      ...
      if (rawConsentEl) {
        activeData.privacy_mode = rawConsentEl.checked
          ? "opt_in_raw"
          : "stats_only";
  ...
  function makeLocalEvidenceReport() {
    ...
    const receipts =
      Array.isArray(evidence.receipts) && evidence.receipts.length
        ? evidence.receipts
        : [];
    ...
    const risk = Number(
      predictive.drop_off_risk || 100 - Number(symmetry.score || 70),
    );
    const headline =
      topReceipt.claim ||
      (symmetry.label
        ? `${symmetry.label} effort pattern`
        : "Conversation pattern ready");
    const nextMove =
      predictive.next_action ||
      topReceipt.action ||
      "Pick the strongest repeated pattern and ask for one clear change.";
    return {
      relationship_persona: topReceipt.pattern
        ? `${String(topReceipt.pattern).replace(/_/g, " ")} pattern`
        : "Pattern read",
      ...
      attachment_style: {
        ...
        compatibility_note:
          "Local read based on aggregate timing, balance, and repeated receipt signals.",
      },
      key_insights: [
        `${total.toLocaleString()} messages were analysed from ${activeData.input_mode || "chat"} input.`,
        topReceipt.evidence ||
          `Balance reads as ${symmetry.label || "unclear"} from message and word-share patterns.`,
        source.warnings?.[0] ||
          "Confidence depends on message count, timestamps, and source quality.",
      ].filter(Boolean),
      strengths: receipts
        .filter((r) => r.pattern === "repair_attempts")
        .map((r) => r.claim)
        .slice(0, 2),
      growth_areas: receipts
        .filter((r) => r.pattern !== "repair_attempts")
        .map((r) => r.claim)
        .slice(0, 3),
      coaching_advice: nextMove,
      fun_fact: stats.streaks?.longest
        ? `Longest active streak: ${stats.streaks.longest} days.`
        : "The clearest read comes from repeated behavior, not one dramatic message.",
      verdict_summary: {
        headline,
        ...
        confidence:
          source.score !== undefined
            ...
            : scoreText(predictive.confidence || 60),
        ...
        : [
            {
              ...
              evidence:
                "The report has enough aggregate chat data to show balance, timing, and source quality.",
              pattern: "local_stats",
              confidence: "medium",
              action: nextMove,
            },
      ...
      predictive_outlook: {
        ...
        reciprocity_trend:
          predictive.reciprocity_trend || symmetry.label || "unclear",
        repair_likelihood: predictive.repair_likelihood || "unclear",
        drop_off_risk:
          predictive.drop_off_risk ?? Math.max(5, Math.min(95, risk)),
        conflict_recurrence_risk:
          predictive.conflict_recurrence_risk || "needs AI context",
      ...
      ai_insight: {
        ...
        reality_check:
          topReceipt.evidence ||
          "This is the local evidence read. Generate AI for a deeper emotional interpretation.",
        recent_shift: predictive.trend
          ? `Recent trend appears ${predictive.trend}.`
          : "Recent shift uses weekly volume, sentiment, and reply timing when timestamps are available.",
        red_flags: receipts
          .filter(
            (r) => r.confidence === "high" && r.pattern !== "repair_attempts",
          )
          .map((r) => r.claim)
          .slice(0, 3),
        green_flags: receipts
          .filter((r) => r.pattern === "repair_attempts")
          .map((r) => r.claim)
          .slice(0, 3),
  ...
  function renderAiReport(report, options = {}) {
    ...
    const healthScore = scoreValue(
      Number(ai.health_score) || Number(report?.overall_health_score) || 0,
    );
    ...
    const predictive =
      ...
      activeData.evidence_pack?.predictive_outlook ||
      {};
    const receipts =
      Array.isArray(report?.receipts) && report.receipts.length
        ? report.receipts
        : activeData.evidence_pack?.receipts || [];

    setText(
      "verdict-headline",
      verdict.headline || ai.dynamic_title || "Pattern read complete",
    );
    setText(
      "verdict-risk",
      verdict.risk_level ||
        (Number(predictive.drop_off_risk) >= 65
          ? "High"
          : Number(predictive.drop_off_risk) >= 40
            ? "Medium"
            : "Low"),
    );
    setText(
      "verdict-confidence",
      verdict.confidence
        ? escapeHTML(String(verdict.confidence)).replace(/\b100%/g, "95+%")
        : predictive.confidence
          ? scoreText(predictive.confidence)
          : scoreText(activeData.source_quality?.score || 60),
    );
    setText(
      "verdict-next-move",
      verdict.best_next_move ||
        predictive.next_action ||
        report?.coaching_advice ||
        "",
    );

    const receiptEl = document.getElementById("receipt-cards");
    if (receiptEl) {
      receiptEl.innerHTML = receipts
        .slice(0, 6)
        .map(
          (r, index) => `
    ...
    setText(
      "ai-persona",
      report?.relationship_persona ? `"${report.relationship_persona}"` : "",
    );
    ...
    if (ring) {
      setTimeout(() => {
        ring.style.setProperty("--score", String(healthScore));
      }, 100);
    }
    if (scoreNum)
      scoreNum.textContent = healthScore
        ? scoreText(healthScore, { suffix: "", plusAtMax: true })
        : "--";
    ...
    setText(
      "compatibility-score",
      compatScore
        ? scoreText(compatScore, { suffix: "", plusAtMax: true })
        : "--",
    );
    ...
    if (commBar)
      setTimeout(() => {
        commBar.style.width = barWidth(commBalance);
      }, 200);
    ...
    if (red)
      red.innerHTML = (ai.red_flags || [])
        .map((f) => `<li>${escapeHTML(f)}</li>`)
        .join("");
    if (green)
      green.innerHTML = (ai.green_flags || [])
        .map((f) => `<li>${escapeHTML(f)}</li>`)
        .join("");
    ...
    if (strengthsEl)
      strengthsEl.innerHTML = (report?.strengths || [])
        .map(
          (s) =>
            `<li style="padding-left:1.25rem;position:relative"><span style="position:absolute;left:0">âœ…</span>${escapeHTML(s)}</li>`,
        )
        .join("");
    if (growthEl)
      growthEl.innerHTML = (report?.growth_areas || [])
        .map(
          (g) =>
            `<li style="padding-left:1.25rem;position:relative"><span style="position:absolute;left:0">ðŸŽ¯</span>${escapeHTML(g)}</li>`,
        )
        .join("");
    ...
    if (weeks.length >= 2) {
      const recentSentiments = weeks
        .slice(-3)
        .map((w) => w.mean_sentiment || 0);
      const earlierSentiments = weeks
        .slice(0, Math.max(1, weeks.length - 3))
        .map((w) => w.mean_sentiment || 0);
      const recentAvg =
      ...
      if (forecastEl) {
        forecastEl.textContent =
          ai.recent_shift ||
          `Based on ${weeks.length} weeks of data, the emotional trajectory is ${diff > 0.1 ? "trending positively" : diff < -0.1 ? "showing signs of decline" : "holding steady"}.`;
      }
    }
    ...
    if (predictiveMetrics) {
      ...
      predictiveMetrics.innerHTML = [
        metric(
          "Stability",
          predictive.stability !== undefined
            ? scoreText(predictive.stability)
            : "--",
        ),
        metric(
          "Drop-off risk",
          predictive.drop_off_risk !== undefined
            ? scoreText(predictive.drop_off_risk)
            : "--",
        ),
        ...
        metric(
          "Confidence",
          predictive.confidence !== undefined
            ? scoreText(predictive.confidence)
            : "--",
        ),
      ].join("");
    ...
    if (forecastEl && predictive.next_action) {
      forecastEl.textContent =
        `${ai.recent_shift || ""} ${predictive.next_action}`.trim();
    }
    ...
    if (actionItemsEl) {
      const actions = [];
      if (predictive.next_action)
        actions.push({
          action: predictive.next_action,
          impact:
            Number(predictive.drop_off_risk || 0) >= 65 ? "high" : "medium",
        });
      receipts
        .slice(0, 2)
        .forEach((r) =>
          actions.push({
            action: r.action,
            impact: r.confidence === "high" ? "high" : "medium",
          }),
        );
      if (report?.coaching_advice)
        actions.push({ action: report.coaching_advice, impact: "high" });
      (report?.growth_areas || []).forEach((g) =>
        actions.push({ action: g, impact: "medium" }),
      );
      actionItemsEl.innerHTML = actions
        .slice(0, 3)
        .map(
          (item) => `
  ...
  const generateAiInsights = async () => {
    ...
    try {
      const payload = {
        ...
        raw_excerpt_pack:
          activeData.privacy_mode === "opt_in_raw"
            ? activeData.raw_excerpt_pack || null
            : null,
        compare_data: isCompareMode
          ? {
              a: dataA.stats,
              b: dataB.stats,
              nameA: dataA.partner_name,
              nameB: dataB.partner_name,
            }
          : null,
        provider: getStoredProvider(),
        api_key: sessionStorage.getItem("_llm_token")
          ? decodeURIComponent(
              escape(atob(sessionStorage.getItem("_llm_token"))),
            )
          : "",
      };
      ...
      const errorMsg =
        e.message && e.message.includes("Free tier limit")
  ...
  document.getElementById("regenerateBtn")?.addEventListener("click", () => {
    ...
    document
      .getElementById("ai-permission-container")
      ?.classList.remove("hidden");
    generateAiInsights();
  });
  ...
  if (
    isSharedMode &&
    window.llmReport &&
    Object.keys(window.llmReport).length > 0
  ) {
  ...
  chatForm?.addEventListener("submit", async (e) => {
    ...
    const chatCount = parseInt(
      sessionStorage.getItem("_chat_count") || "0",
      10,
    );
    if (chatCount >= 50) {
      ...
      errMsg.textContent =
        "Session limit reached (50 messages). Refresh the page to continue.";
    ...
    try {
      ...
      coachMsg.style.cssText =
        "align-self: flex-start; background: var(--white); border: 2px solid var(--black); box-shadow: 2px 2px 0 0 var(--black); padding: 0.5rem 1rem; border-radius: 12px 12px 12px 0; max-width: 85%; line-height: 1.5; font-weight: 500;";
      coachMsg.innerHTML = `<strong>Coach:</strong> ${formattedText}`;
      historyEl.appendChild(coachMsg);

      window.coachingChatHistory.push({
        role: "assistant",
        content: data.text,
      });
    } catch (err) {
...
async function downloadWrappedCard() {
  ...
  try {
    ...
    setVal(
      "share-title",
      report.ai_insight?.dynamic_title || report.relationship_persona,
    );
    ...
    setVal(
      "share-total",
      (stats.messages?.ME + stats.messages?.PARTNER || 0).toLocaleString(),
    );
    setVal("share-duration", stats.duration || "--");
    setVal(
      "share-red",
      report.ai_insight?.red_flags ? report.ai_insight.red_flags[0] : "--",
    );
    setVal(
      "share-green",
      report.ai_insight?.green_flags ? report.ai_insight.green_flags[0] : "--",
    );
    setVal(
      "share-verdict",
      report.ai_insight?.brutal_verdict || "The vibe is set.",
    );
    ...
    document.getElementById("share-bar-partner").style.width =
      100 - meBarPct + "%";

    // Provide a stats object based solely on anonymous statistics
    const statsPayload = {
      health_score: formatHeuristicScore(
        report.ai_insight?.health_score || report.overall_health_score || 0,
        { suffix: "", plusAtMax: true },
      ),
      top_insight:
        report.ai_insight?.brutal_verdict || "Your chat data has been decoded.",
    };
  ...
  try {
    ...
    const shareData = {
      ...
      evidence_pack: {
        ...
        predictive_outlook:
          window.activeData.evidence_pack?.predictive_outlook || null,
      },
...
function generateShareCard(analysisData) {
  ...
  const healthScore = formatHeuristicScore(
    ai.health_score ||
  ...
  const predictive =
    report.predictive_outlook || active.evidence_pack?.predictive_outlook || {};
  const dropOffRisk =
    predictive.drop_off_risk !== undefined
      ? formatHeuristicScore(predictive.drop_off_risk)
      : null;
  const init = stats.initiator_ratio || {};
  ...
  const redFlag =
    ai.red_flags?.[0] ||
    report.growth_areas?.[0] ||
    "No major red flag detected.";
  const topReceipt =
    report.receipts?.[0] || active.evidence_pack?.receipts?.[0];
  const greenFlag =
    topReceipt?.claim ||
    ai.green_flags?.[0] ||
    report.strengths?.[0] ||
    "There is enough signal to read the vibe.";
  const title = ai.dynamic_title || report.relationship_persona || "Vibe Check";
  const finalWord =
    ai.brutal_verdict || analysisData.top_insight || "The chat has spoken.";
  ...
  drawMetric(
    ctx,
    "Balance",
    stats.symmetry?.label || `${mePct}/${partnerPct}`,
    560,
    930,
    "#eee3ff",
  );
  ...
  wrapText(
    ctx,
    `${active.my_name || "You"} ${mePct}% / ${active.partner_name || "Them"} ${partnerPct}% of ${total.toLocaleString()} messages`,
    112,
    1162,
    856,
    36,
  ...
  ctx.font = "900 28px Arial, sans-serif";
  ctx.fillText(
    `PREDICTION: ${dropOffRisk ? dropOffRisk + " DROP-OFF RISK" : "WATCH THE TREND"}`,
    112,
    1700,
  );
...
function drawPill(ctx, text, x, y, bg, fg, size) {
  ...
  ctx.fillRect(
    x,
    y,
    Math.min(856, 34 + String(text).length * size * 0.58),
    size + 26,
  );
  ctx.fillStyle = fg;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (shareImageBtn) {
    shareImageBtn.addEventListener("click", () => {
      ...
      const statsPayload = {
        health_score: formatHeuristicScore(
          window.llmReport?.ai_insight?.health_score ||
            window.llmReport?.overall_health_score ||
            0,
          { suffix: "", plusAtMax: true },
        ),
        top_insight:
          window.llmReport?.ai_insight?.brutal_verdict ||
          "Your chat data has been decoded.",
      };
      const imageData = generateShareCard(statsPayload);
      const link = document.createElement("a");
      link.download = "my-algorithm-results.png";
