...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderSocialDynamics = (stats) => {
        ...
        const update = (suffix, data) => {
            ...
            if (pills) pills.innerHTML = (data.highlights || []).map(h => {
                return `<span class="pill-label pill-label--pink" style="font-size:0.6rem">${escapeHTML(h.label)}: ${h.count}</span>`;
            }).join('');
    ...
    const renderEmoji = (stats) => {
        ...
        container.innerHTML = emojis.map(e => {
            return `<div class="flex align-center gap-3"><span>${escapeHTML(e.emoji)}</span><div class="flex-1 h-2 bg-cream rounded-full overflow-hidden"><div class="h-full bg-pink" style="width:${(e.count / emojis[0].count * 100)}%"></div></div></div>`;
        }).join('');
    };
    ...
    const renderHumor = (stats) => {
        ...
        container.innerHTML = `
            <div class="flex justify-between mb-2"><span>${escapeHTML(activeData.my_name)} Laughs</span><span class="font-black">${h.ME}</span></div>
            <div class="flex justify-between mb-4"><span>${escapeHTML(activeData.partner_name)} Laughs</span><span class="font-black">${h.PARTNER}</span></div>
    ...
    const renderGhosting = (stats) => {
        ...
        container.innerHTML = `
            ...
            <div class="flex justify-between mb-1"><span>${escapeHTML(activeData.my_name)} broke silence</span><span class="font-black">${g.breakers.ME} times</span></div>
            <div class="flex justify-between"><span>${escapeHTML(activeData.partner_name)} broke silence</span><span class="font-black">${g.breakers.PARTNER} times</span></div>
        `;
    };
    ...
    const renderWordCloud = (stats) => {
        ...
        container.innerHTML = `<div class="flex flex-wrap justify-center p-4 gap-4">${words.slice(0, 20).map(w => {
            return `<span style="font-size:${0.8 + (w.count/max)*1.5}rem; font-weight:900; opacity:${0.4 + (w.count/max)*0.6}">${escapeHTML(w.word)}</span>`;
        }).join('')}</div>`;
    };
    ...
    chatForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            errMsg.innerHTML = `Error: ${escapeHTML(err.message)}`;
