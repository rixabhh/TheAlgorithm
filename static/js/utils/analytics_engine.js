...
class AnalyticsEngine {
  constructor() {
    ...
    this.STRESS_RE =
      /work|tired|sad|stressed|deadline|exhausted|unhappy|worry|anxious|sick|hard time|presure|dukh|pareshan|tension|thak|beemar|tension|dard|rona|pareshani|mushkil|gadbad/i;
    this.AFFIRMATIVE_RE =
      /love|thanks|happy|we|miss|appreciate|glad|proud|beautiful|care|pyaar|shukriya|accha|theek|badhiya|jaan|sona|shona|dil|dhanyawad|sundar|mast|swagat|namaste/i;
    this.DISMISSIVE_RE =
      /whatever|fine|okay|sure|k|ok|busy|idk|anyway|thik h|hmmm|hmm|okey|theek hai|achha hai|thik hai|chal|hat|pata nahi|pata nhi|ignore|bye/i;

    // Topic Mix Regexes
    this.LOGISTICS_RE =
      /dinner|lunch|bill|home|work|done|todo|buy|shop|cleaning|khana|ghar|paisa|office|market|sabzi|payment|dukan|dukaan/i;
    this.EXTERNAL_RE =
      /friends|party|movie|news|gym|weather|job|dost|bahar|ghumna|travel|trip|planning|shadi|shaadi|office|colleague/i;
    this.CONFLICT_RE =
      /sorry|why|fight|angry|stop|listen|mean|hurt|annoyed|galti|kyu|gussa|chup|kyun|ladai|jhagda|badtameez|bewafa|jhoot|jut/i;
    this.INTIMACY_RE =
      /love|miss|baby|darling|honey|kiss|hug|beautiful|forever|miss u|pyaar|jaan|shona|ummah|jaaneman|mohabbat|ishq|sath/i;

    // Sentiment Sets (Hinglish Included)
    this.POS_WORDS = new Set([
      "love",
      "happy",
      "great",
      "awesome",
  ...
  runPipeline(messages, connectionType) {
    // removed default value to satisfy unused rule if possible, or just keep it
    ...
    const validMessages = messages
      ...
      .filter(
        (m) =>
          m.sender && m.text !== undefined && !isNaN(m.timestamp.getTime()),
      )
      .sort((a, b) => a.timestamp - b.timestamp);

    if (!validMessages.length) return null;

    const processed = validMessages.map((m) => ({
      ...m,
      textLower: (m.text || "").toLowerCase(),
      words: this.tokenize(m.text || ""),
      chars: (m.text || "").length,
    }));
    ...
    const totalMessages = msgDist.ME + msgDist.PARTNER;
    ...
    return {
      ...
      words: {
        ME: powerInfo.me_word_count,
        PARTNER: powerInfo.partner_word_count,
      },
      chars: {
        ME: powerInfo.me_char_count,
        PARTNER: powerInfo.partner_char_count,
      },
      threads: threads,
      duration: duration,
      laughter: laughter,
      ...
      caps_lock: capsLock,
      streaks: {
        longest: streakInfo.max_streak,
        current: streakInfo.current_streak || 0,
        active_pct: streakInfo.active_pct,
      },
      ...
      sentiment_summary: {
        ...
        overall_mean: sentimentInfo.overallMean,
      },
    };
  }
  ...
  calculateLatency(messages) {
    for (let i = 1; i < messages.length; i++) {
      ...
      if (current.sender !== prev.sender) {
        if (gapMins <= 1440)
          current.latencyMins = gapMins; // Within 24 hours
  ...
  calculateDoubleTexts(messages) {
    ...
    for (let i = 1; i < messages.length; i++) {
      ...
      if (
        ...
        current.gapMins >= DOUBLE_TEXT_THRESHOLD
      ) {
  ...
  calculateStreaks(messages) {
    ...
    return {
      ...
      active_pct: Math.min(
        100,
        Math.round((activeDays.size / (totalDaysRange || 1)) * 100),
      ),
    };
  }
  ...
  calculateLexicalDiversity(messages) {
    ...
    const getPct = (words) =>
      words.length
        ? ((new Set(words).size / words.length) * 100).toFixed(1)
        : 0;
    return {
      ME: {
        pct: getPct(meWords),
        total: meWords.length,
        unique: new Set(meWords).size,
      },
      PARTNER: {
        pct: getPct(pWords),
        total: pWords.length,
        unique: new Set(pWords).size,
      },
    };
  }
  ...
  calculateSymmetryScore(init, power) {
    ...
    const initBalance = 1 - Math.abs(0.5 - (init.me_ratio ?? 0.5)) * 2;
    const powerRatio = power.power_ratio || 1;
    const powerBalance = powerRatio > 1 ? 1 / powerRatio : powerRatio;
  ...
  calculateBehavioralTraits(messages) {
    ...
    const GRATITUDE_RE =
      /thanks|thank you|patience|appreciate|shukriya|dhanyawad/i;
    const COMPLIMENT_RE =
      /nice|great|good job|proud|smart|beautiful|handsome|sundar|badhiya/i;
    const QUESTION_RE =
      /\?|how|why|what|when|where|who|kya|kyu|kab|kaise|kaha/i;
    ...
    const finalize = (u) => {
      ...
      const densityScore = (count, strongDensity, baseline = 0.01) => {
        ...
        const score =
          22 + (density / Math.max(0.01, strongDensity - baseline)) * 68;
      ...
      return {
        scores: {
          ...
          politeness: blend(
            densityScore(u.greetings, 0.14, 0.01),
            densityScore(u.gratitude, 0.08, 0),
          ),
          warmth: blend(
            densityScore(u.compliments, 0.08, 0),
            densityScore(u.affirmative, 0.22, 0.025),
          ),
        ...
        summary: highlights.length
          ? highlights
              ...
              .map(
                (h) =>
                  `${h.label}: ${h.count} signals across ${u.total} messages.`,
              )
  ...
  applySentiment(messages) {
    ...
    return {
      ...
      overallMean: count > 0 ? total / count : 0,
    };
  }
  ...
  extractEmojiStats(messages) {
    ...
    return {
      ...
      total:
        Object.values(stats.ME).reduce((a, b) => a + b, 0) +
        Object.values(stats.PARTNER).reduce((a, b) => a + b, 0),
    };
  }
  ...
  calculateInitiatorRatio(messages) {
    ...
    if (messages.length > 0)
      messages[0].sender === "ME" ? meInits++ : partnerInits++;
    for (let i = 1; i < messages.length; i++) {
      const m = messages[i];
      if (m.gapMins >= GAP_THRESHOLD)
        m.sender === "ME" ? meInits++ : partnerInits++;
      if (m.latencyMins !== null && m.latencyMins !== undefined) {
        m.sender === "ME"
          ? meLats.push(m.latencyMins)
          : pLats.push(m.latencyMins);
      }
    }
  ...
  calculatePowerDynamics(messages) {
    ...
    for (const m of messages) {
      if (m.sender === "ME") {
        meWords += m.words.length;
        meChars += m.chars;
      } else {
        partnerWords += m.words.length;
        partnerChars += m.chars;
      }
    }
    return {
      me_word_count: meWords,
      partner_word_count: partnerWords,
      me_char_count: meChars,
      partner_char_count: partnerChars,
      power_ratio:
        partnerWords > 0 ? meWords / partnerWords : meWords > 0 ? 2 : 1.0,
    };
  }
  ...
  calculateSupportGap(messages) {
    ...
    for (const m of messages) {
      ...
      if (m.sender === "ME" && activeStressPartnerTs) {
        if (
          m.timestamp - activeStressPartnerTs <= SUPPORT_WINDOW &&
          m.words.length > 2
        ) {
          partnerSupported++;
          activeStressPartnerTs = null;
        ...
        if (
          m.timestamp - activeStressMeTs <= SUPPORT_WINDOW &&
          m.words.length > 2
        ) {
          meSupported++;
    ...
    return {
      ...
      PARTNER: {
        stress_count: partnerStress,
        support_received: partnerSupported,
      },
    };
  }
  ...
  calculateSupportScore(supportInfo) {
    const totalStress =
      supportInfo.ME.stress_count + supportInfo.PARTNER.stress_count;
    const totalSupp =
      supportInfo.ME.support_received + supportInfo.PARTNER.support_received;
  ...
  calculateMirroring(messages) {
    ...
    const meTop = Object.keys(meFreq)
      .sort((a, b) => meFreq[b] - meFreq[a])
      .slice(0, 30);
    const pTop = new Set(
      Object.keys(pFreq)
        .sort((a, b) => pFreq[b] - pFreq[a])
        .slice(0, 30),
    );
    let common = 0;
    for (const w of meTop) if (pTop.has(w)) common++;
    const denominator = Math.min(30, Math.max(meTop.length, pTop.size));
    return denominator ? Math.round((common / denominator) * 100) : 0;
  }
  ...
  calculateTopicMix(messages) {
    ...
    return {
      breakdown: topics,
      top_topic: sorted[0][1] > 0 ? sorted[0][0] : "Life Talk",
    };
  }
  ...
  aggregateWeekly(messages) {
    ...
    for (const m of messages) {
      ...
      if (m.latencyMins !== null && m.latencyMins !== undefined)
        weeks[key].latencies.push(m.latencyMins * 60);
      weeks[key].sentiments.push(m.sentiment || 0);
      if (m.sender === "ME") weeks[key].me_sentiments.push(m.sentiment || 0);
      if (m.sender === "PARTNER")
        weeks[key].partner_sentiments.push(m.sentiment || 0);
    }
    return Object.values(weeks)
      ...
      .map((w) => ({
        ...
        me_sentiment: this._mean(w.me_sentiments) || 0,
        partner_sentiment: this._mean(w.partner_sentiments) || 0,
      }));
  }
  ...
  calculateRiskScore(weeks) {
    ...
    return weeks.map((w) => {
      const risk =
        0.5 * ((1 - w.mean_sentiment) / 2) +
        0.3 * (w.median_latency / maxLat) +
        0.2 * (1 - w.volume / maxVol);
