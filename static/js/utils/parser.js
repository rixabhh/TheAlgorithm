...
class ChatParser {
  constructor() {
    ...
    this.WA_MSG_PATTERN =
      /^\[?(?<date>\d{1,2}[/-]\d{1,2}[/-]\d{2,4})[,\s]+(?<time>\d{1,2}:\d{2}(?::\d{2})?(?:\s*[aApP]\.?[mM]\.?)?)(?:\]\s*|\s*[-–]\s+)(?<sender>[^:]+):\s*(?<text>.*)$/;
    this.WA_SYS_PATTERN =
      /^\[?(?<date>\d{1,2}[/-]\d{1,2}[/-]\d{2,4})[,\s]+(?<time>\d{1,2}:\d{2}(?::\d{2})?(?:\s*[aApP]\.?[mM]\.?)?)(?:\]\s*|\s*[-–]\s+)(?<sys_msg>Messages and calls are end-to-end encrypted.*|.*changed their phone number|.*joined using an invite link|.*left|.*changed the subject to|.*changed the group description|.*You deleted this message.*)$/;
    ...
    this.TG_MEDIA_PATTERN =
      /<div class="media_wrap[^>]*>.*?<div class="title">\s*([^<]+)\s*<\/div>.*?<div class="status">\s*([^<]+)\s*<\/div>/gs;

    this.SIGNAL_MSG_PATTERN =
      /^\[?(?<date>\d{4}-\d{2}-\d{2}) (?<time>\d{2}:\d{2})\]? (?<sender>[^:]+):\s+(?<text>.*)$/;
  ...
  detect(content, fileName = "") {
    ...
    if (lowerName.endsWith(".json")) {
      ...
      if (content.includes('"author":') && content.includes('"timestamp":'))
        return "Discord";
      if (
        content.includes('"sender_name":') &&
        content.includes('"timestamp_ms":')
      )
        return "Instagram";
      return "JSON";
    }
  ...
  parseTelegram(htmlData) {
    ...
    for (let i = 1; i < blocks.length; i++) {
      ...
      // Date
      let dateMatch =
        block.match(this.TG_DATE_PATTERN) ||
        block.match(this.TG_DATE_FALLBACK_PATTERN);
  ...
  standardizeEntities(messages, myName, partnerName) {
    ...
    // Sort by frequency
    const sortedSenders = Object.keys(counts).sort(
    ...
    if (
      mappedRoles().has("ME") &&
      !mappedRoles().has("PARTNER") &&
      remainingTop.length === 1
    ) {
