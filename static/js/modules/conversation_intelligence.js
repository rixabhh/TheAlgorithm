...
class ConversationIntelligence {
  ...
  parseLooseLines(text, sourceType) {
    ...
    const withDate =
      /^\[?(?<date>\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|\d{4}-\d{2}-\d{2})[,\s]+(?<time>\d{1,2}:\d{2}(?::\d{2})?(?:\s*[aApP]\.?[mM]\.?)?)\]?\s*(?:[-–]\s*)?(?<sender>[^:]{1,80}):\s*(?<text>.*)$/;
    ...
    for (const line of lines) {
      ...
      if (messages.length && lastSender) {
        ...
        messages[messages.length - 1].confidence = Math.min(
          messages[messages.length - 1].confidence || 0.6,
          0.62,
        );
      }
    }
  ...
  async readScreenshots(files, onProgress) {
    ...
    if (!imageFiles.length)
      return {
        text: "",
        confidence: 0,
        warnings: ["No screenshot images selected."],
      };
    ...
    for (const file of imageFiles) {
      onProgress?.(
        `Reading screenshot ${completed + 1}/${imageFiles.length}...`,
        Math.round((completed / imageFiles.length) * 80),
      );
    ...
    const confidence = imageFiles.length
      ? Math.round(confidenceTotal / imageFiles.length)
      : 0;
    const warnings = [];
    if (confidence < 65)
      warnings.push(
        "OCR confidence is low. Review and correct the extracted text before analysing.",
      );
  ...
  ensureTesseract() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () =>
        reject(
          new Error(
            "OCR engine could not be loaded. Paste the screenshot text manually to continue.",
          ),
        );
  ...
  standardize(
    ...
    const mapped = this.parser.standardizeEntities(
      enriched,
      myName,
      partnerName,
    );
  ...
  assessSourceQuality(messages, inputMode, extra = {}) {
    ...
    const lowConfidenceCount = messages.filter(
      (m) => Number(m.confidence || 0) < 0.7,
    ).length;
    const missingTimestampPenalty =
      inputMode === "paste" || inputMode === "transcript" ? 10 : 0;
    const confidenceAvg = parsedCount
      ? messages.reduce((sum, m) => sum + Number(m.confidence || 0.7), 0) /
        parsedCount
      : 0;
    ...
    if (inputMode === "paste" || inputMode === "transcript") {
      warnings.push(
        "Some timestamps may be synthetic, so timing-based predictions are less certain.",
      );
    }
  ...
  buildEvidencePack(messages, stats, sourceQuality) {
    ...
    messages.forEach((m) => {
      if (bySender[m.sender]) bySender[m.sender].push(m);
    });

    const countPattern = (sender, regex) =>
      bySender[sender].filter((m) => regex.test(m.text || "")).length;
    ...
    const boundaryPressure =
      countPattern(
        "PARTNER",
        /\b(you always|you never|if you cared|prove|your fault|because of you|stop overreacting)\b/i,
      ) +
      countPattern(
        "ME",
        /\b(you always|you never|if you cared|prove|your fault|because of you|stop overreacting)\b/i,
    ...
    const repairAttempts =
      countPattern(
        "ME",
        /\b(sorry|my bad|let's talk|can we fix|i understand|i hear you)\b/i,
      ) +
      countPattern(
        "PARTNER",
        /\b(sorry|my bad|let's talk|can we fix|i understand|i hear you)\b/i,
    ...
    if (meLatency && partnerLatency) {
      ...
      const ratio =
        Math.max(meLatency, partnerLatency) /
        Math.max(1, Math.min(meLatency, partnerLatency));
      if (ratio >= 1.8) {
        receipts.push(
          this.receipt(
    ...
    if (support.ME?.stress_count || support.PARTNER?.stress_count) {
      const meSupportRate = support.ME.stress_count
        ? support.ME.support_received / support.ME.stress_count
        : 1;
      const partnerSupportRate = support.PARTNER.stress_count
        ? support.PARTNER.support_received / support.PARTNER.stress_count
        : 1;
    ...
    const prediction = this.buildPredictiveOutlook(
      stats,
      sourceQuality,
      receipts,
    );
  ...
  findUnansweredQuestions(messages) {
    ...
    for (let i = 0; i < messages.length - 1; i++) {
      ...
      if (
        !/\?|\b(why|what|when|where|how|kya|kyu|kab|kaise)\b/i.test(
          current.text || "",
        )
      )
        continue;
      ...
      const deflect =
        /\b(idk|later|leave it|forget it|whatever|hmm|ok|seen)\b/i.test(
          next.text || "",
        );
  ...
  findDelayedAfterConflict(messages) {
    const conflictRe =
      /\b(fight|angry|hurt|upset|ignore|ignored|sorry|rude|toxic|lie|lied|cheat|trust|problem)\b/i;
    let count = 0;
    for (let i = 0; i < messages.length - 1; i++) {
      const current = messages[i];
      const next = messages[i + 1];
      ...
      const gapHours =
        (new Date(next.timestamp) - new Date(current.timestamp)) / 3600000;
      if (gapHours >= 6) count++;
    }
    return { count };
  }
  ...
  buildPredictiveOutlook(stats, sourceQuality, receipts) {
    const riskReceipts =
      receipts.filter((r) => r.confidence === "high").length * 18 +
    ...
    if (weeks.length >= 2) {
      ...
      const avg = (arr) =>
        arr.reduce((sum, w) => sum + Number(w.mean_sentiment || 0), 0) /
        arr.length;
    ...
    const riskBase =
      Math.max(0, 100 - symmetryScore) +
      riskReceipts +
      (trend === "declining" ? 18 : 0);
    const dropOffRisk = Math.max(5, Math.min(95, Math.round(riskBase)));
    const stability = Math.max(
      5,
      Math.min(
        95,
        Math.round(100 - dropOffRisk + (trend === "improving" ? 12 : 0)),
      ),
    );
    const confidence = Math.round(
      sourceQuality.score * 0.65 +
        Math.min(100, Number(stats?.total_messages || 0) / 5) * 0.35,
    );
    return {
      trend,
      ...
      reciprocity_trend:
        symmetryScore >= 75
          ...
          : symmetryScore >= 55
            ? "uneven but workable"
            : "imbalanced",
      repair_likelihood: receipts.some((r) => r.pattern === "repair_attempts")
        ? "medium-high"
        : "unclear",
      drop_off_risk: dropOffRisk,
      conflict_recurrence_risk: receipts.some(
        (r) =>
          r.pattern === "conflict_delay" || r.pattern === "boundary_pressure",
      )
        ? "elevated"
        : "normal",
      next_action:
        dropOffRisk >= 65
          ? "Have one direct repair conversation and watch the follow-through."
          : "Pick one repeated pattern and make a small request around it.",
    };
  }
