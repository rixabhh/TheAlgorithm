...
## 2026-03-30 - [Index Alignment & Regex Pre-compilation]
**Learning:** Scrambled index labels in DataFrames (e.g., after `sort_values` without `ignore_index=True`) trigger expensive O(N) index alignment logic in downstream vectorized operations. This alignment can also cause data corruption if the scrambled index is a permutation of 0..N-1, as it aligns by label rather than position. Additionally, joining and compiling keyword lists into regexes on every request adds significant CPU overhead.
**Action:** Use `ignore_index=True` in `sort_values` for atomic sorting and index resetting. Centralize `df.reset_index(drop=True)` at the analytics pipeline entry before deriving secondary Series (e.g., `text_str`, `text_lower`) to ensure perfect alignment. Move all static keyword lists to the module level and pre-compile them as `re.Pattern` objects for high-speed matching.
